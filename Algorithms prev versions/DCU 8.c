...
//HELPER FUNCTIONS
  ...
  void mainMenu (superArray* superArrays, bool* running, superSort* superSorts, node** lists, stackNode** stacks, queueNode** queues);
...
void mainMenu (superArray* superArrays, bool* running, superSort* superSorts, node** lists, stackNode** stacks, queueNode** queues){
  ...
  if (validInput == 1)
    manageArrays(superArrays, superSorts);
...
int main(){
  ...
  //main program loop
  while (running) mainMenu(superArrays, &running, superSorts, lists, stacks, queues);
